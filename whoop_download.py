...
class whoop_login:
    ...
    def __init__(
        self, auth_code=None, whoop_id=None, current_datetime=datetime.utcnow()
    ):
    ...
    def pull_sleep_main(self, sleep_id):
        ...
        sleep = self.pull_api(
            "https://api-7.whoop.com/users/{}/sleeps/{}".format(athlete_id, sleep_id)
        )
    ...
    def get_activities_all(self):
        ...
        if self.start_datetime:
            ...
            act_data = pd.json_normalize(
                data[data["strain.workouts"].apply(len) > 0]["strain.workouts"].apply(
                    lambda x: x[0]
                )
            )
    ...
    def get_hr_all(self, df=False):
        ...
        if self.start_datetime:
            ...
            start_time = "T00:00:00.000Z"
            intervals = rrule.rrule(
                freq=WEEKLY, interval=1, until=self.current_datetime, dtstart=start_date
            )
            date_range = [
                [
                    d.strftime("%Y-%m-%d") + start_time,
    ...
    def get_keydata_timeframe(self, start, end):
        """
        ...
        if st > e:
            ...
            if self.auth_code:
                end_time = "T23:59:59.999Z"
                start_time = "T00:00:00.000Z"
                intervals = rrule.rrule(DAILY, interval=1, until=e, dtstart=st)
                date_range = [
                    [
                        d.strftime("%Y-%m-%d") + start_time,
                        d.strftime("%Y-%m-%d") + end_time,
                    ]
                    for d in intervals
                    if d <= e
                ]
    ...
    def get_activities_timeframe(
        self, start, end=datetime.strftime(datetime.utcnow(), "%Y-%m-%d")
    ):
        """
        Activity data is pulled through the get_keydata functions so if the data pull is present, this function
        just transforms the activity column into a dataframe of activities, where each activity is a row.
    ...
    def get_sleep_events_timeframe(
        self, start, end=datetime.strftime(datetime.utcnow(), "%Y-%m-%d")
    ):
        """
        ...
        if st > e:
            ...
            if self.auth_code:
                ...
                if self.all_sleep_events is not None:
                    ...
                    if self.all_sleep is not None:
                        ...
                        time_sleep_events = pd.concat(
                            [
                                ...
                                for events, sleep in zip(
                                    time_sleep["events"], time_sleep["activityId"]
                                )
                            ]
                        )
                    else:
                    ...
                    time_sleep_events["total_minutes"] = time_sleep_events.apply(
                        lambda x: (
                            x["during.upper"] - x["during.lower"]
                        ).total_seconds()
                        / 60.0,
                        axis=1,
                    )
    ...
    def get_hr_timeframe(
        self, start, end=datetime.strftime(datetime.utcnow(), "%Y-%m-%d"), df=False
    ):
        """
